...
fn generate_items_jsonl(n: usize, namespace: u16, status: &str) -> String {
    let mut lines = Vec::new();
    lines.push("{\"schema_version\":1}".to_string());

    let now = "2026-02-24T12:00:00Z";
    for i in 0..n {
        let id = format!("018f2b1c-4d5e-7abc-8123-{namespace:04x}{i:08x}");
...
fn large_store_2000_active_items() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let content = generate_items_jsonl(2000, 1, "todo");
    ...
    let output = project.run_tg(&["--json", "show", "018f2b1c-4d5e-7abc-8123-000100000000"]);
    assert!(output.status.success());
...
fn large_store_5000_archive_items() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let content = generate_items_jsonl(5000, 2, "done");
    ...
    let output = project.run_tg(&["--json", "show", "018f2b1c-4d5e-7abc-8123-000200000000"]);
    assert!(output.status.success());
...
fn large_store_mixed_2000_active_5000_archive() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Write active items
    let tasks_path = project.path().join(".task-golem/tasks.jsonl");
    let active_content = generate_items_jsonl(2000, 1, "todo");
    fs::write(&tasks_path, active_content)?;

    // Write archive items (using different IDs to avoid collisions)
    let archive_path = project.path().join(".task-golem/archive.jsonl");
    let archive_content = generate_items_jsonl(5000, 2, "done");
